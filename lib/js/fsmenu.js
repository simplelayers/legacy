...
FSMenu.prototype.hide = function (mN) {
    with(this) {
		if(this.hideOnRollout != undefined){
			if(this.hideOnRollout && (null != menus[mN].par)){
				if (!menus[mN].par.isRoot){
					clearTimeout(mtsTimer);
					if (menus[mN]) menus[mN].clickOut();
				}
			}
		}
    }
};
FSMenu.prototype.clickHide = function (mN) {
    with(this) {
		clearTimeout(mtsTimer);
		if (menus[mN]) menus[mN].clickOut();
    }
...
FSMenu.prototype.hideAll = function () {
    with(this) {
        for (var m in menus) if (menus[m].visible && !menus[m].isRoot) menus[m].clickHide(true);
    }
};
...
function FSMenuNode(id, isRoot, obj) {
    ...
    this.out = function (evt) {
		if(this.hideOnRollout){
			clickOut();
		}
    };
	this.clickOut = function (evt) {
        with(node) with(obj) {
            if (isNS4 && evt && lyr && lyr.ref) lyr.ref.routeEvent(evt);
            clearTimeout(timer);
            if (!isRoot && hideDelay >= 0) {
                timer = setTimeout(myName + '.menus["' + id + '"].clickHide()', hideDelay);
                if (!nested && par) par.clickOut();
            }
        }
    };
    if (id != 'root') with(this) with(lyr = getLyr(id)) if (ref) {
        ...
        if (obj.nested) {
            ...
            addEvent(ref, 'blur', this.clickOut, 1);
        }
    }
};
...
FSMenuNode.prototype.clickHide = function (forced) {
	this.hide(forced);
};
...
FSMenu.prototype.activateMenu = function (id, subInd) {
    with(this) {
        ...
        for (var i = 0; i < lists.length; i++) {
            ...
            var eClickHide = new Function('e', 'if(e.' + evtProp + '!=false)' + myName + '.clickHide("' + menuID + '")');
            addEvent(a, 'mouseover', eShow);
            addEvent(a, 'mouseout', eHide);
            addEvent(a, 'focus', eShow);
            addEvent(a, 'blur', eClickHide);
            addEvent(a, 'click', new Function('e', 'var s=' + myName + '.showOnClick,m=' + myName + '.menus["' + menuID + '"];if(!((s==1&&' + rootItem + ')||s>=2))return;' + myName + '[m&&m.visible?"clickHide":"show"]("' + menuID + '",this);if(e.cancelable&&e.preventDefault)e.preventDefault();e.' + evtProp + '=false;return false'));
