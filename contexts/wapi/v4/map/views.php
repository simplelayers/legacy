...
function _exec() {
	...
	$views = new ProjectUserViews($userId,System::GetDB());
	...
	switch(ParamUtil::RequiresOne($args,'action','view')) {
		...
		case 'view':
		    ...
		    switch(ParamUtil::Get($args,'type')) {
		        case ProjectUserViews::BOOKMARKED:
		            $results = $views->GetBookMarked($filter,$and);
		            break;
		        case ProjectUserViews::GROUPS:
		            
		            $results = $views->GetGroups($filter,$and);
		            
		            break;
		        case ProjectUserViews::GROUP:
		            ...
		            break;
		        case ProjectUserViews::MINE:
		            
		            $results = $views->GetMine($filter,$and);
		            
		            break;
		        case ProjectUserViews::OWNERS:
		            $results = $views->GetOwners((isset($_REQUEST['min']) ? (int)$_REQUEST['min'] : 1), $filter,$and);
		            break;
		        case ProjectUserViews::SHARED:
		        case ProjectUserViews::OWNER:
