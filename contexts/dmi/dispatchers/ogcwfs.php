...
function _dispatch_ogcwfs($template, $args) {
$user = $args['user'];
$world = $args['world'];
...
$template->assign('onlineresource', "{$world->config['url']}/?do=ogcwfs&project={$project->id}&" );
...
switch(strtolower(@$_GET['request'])) {
    case 'getcapabilities':
        ...
        $template->display('ogcwfs_capabilities.tpl');
        break;
    case 'describefeaturetype':
        ...
        header('Content-type: text/xml');
        $template->display('ogcwfs_featurexsd.tpl');
        break;
    case 'getfeature':
        // check for missing params
        if (!@$_GET['typename']) return print "Missing required parameter: TYPENAME";
        ...
        if (@$_GET['bbox']) {
            ...
            $records = $layer->getRecords(null,'GML');
        }
        ...
        header('Content-type: text/xml');
        $template->display('ogcwfs_gml2.tpl');
        break;
    default:
