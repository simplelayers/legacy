...
function _dispatch_ogcwcs($template, $args) {
$user = $args['user'];
$world = $args['world'];
...
switch(strtolower($_GET['request'])) {
    case 'getcoverage':
        ...
        foreach (explode(',',$_GET['coverage']) as $layerid) {
            $projectlayer = $project->getLayerById($layerid);
        ...
        //header("Content-disposition: attachment; filename=WCS_{$_GET['coverage']}.tif");
        //header('Content-type: image/tiff',true);
        print $mapper->renderStream();
        break;
    case 'getcapabilities':
        $bbox = explode(',',$project->bbox);
        $template->assign('title', $project->name );
        $template->assign('onlineresource', "{$world->config['url']}/wcs/?project={$project->id}&" );
        ...
        $template->display('ogcwcs_capabilities.tpl');
        break;
    default:
