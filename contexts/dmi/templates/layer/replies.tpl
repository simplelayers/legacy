...
<div style="padding:0px;">
	...
	<div style="font-size:10pt;padding:3px 6px;"><a href="#" class="reply">reply</a><!--{if ($owner->id == $user->id || $user->admin || $layer->owner->id == $user->id) && $reply.id}--> &nbsp;&nbsp; <a href="./?do=layer.discussion&id=<!--{$layer->id}-->&del=<!--{$reply.id}-->" class="delete">delete</a><!--{/if}-->
		<form style="display:none;" action="./?do=layer.discussion&id=<!--{$layer->id}-->" method="post">
	...
	<div style="padding-left:24px;border-left:1px dotted #aaa;"><!--{if !empty($replys.children)}--><!--{include file="layer/replies.tpl" layer=$layer replies=$replys.children world=$world user=$user}--><!--{/if}--></div>
</div>
<!--{/foreach}-->